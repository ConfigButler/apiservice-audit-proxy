...
| `e2e-smoke` | No (host runner, CI image via `docker run`) | Pulls the CI container and the application image built by `docker-build`, then runs the live e2e suite inside the CI container (DooD: Docker socket + workspace mounted): `task e2e:test-smoke-full`, `task e2e:test-smoke-backend-ca`, `task e2e:test-audit-gap`, `task e2e:test-impersonation`, and `task e2e:test-impersonation-no-rbac`; runs in parallel with lint/test |
